...
OPT_DESC = {
    '<midi>': ['Midi device e.g. /dev/midi1'],
    ...
    '--note-pressures': [
        'To consider 3 levels of pressure for a note', {
...
def usage():
    """usage"""
    opts_keys = list(OPT_DESC.keys())
    opts_keys.sort(
        key=lambda a: "Z%s" % a if a.startswith('-') else a
    )
    opts = [
        " ".join(
            [o] + list((OPT_DESC[o] + [{}])[1].keys())
        )
        for o in opts_keys
    ]
    print(
    ...
    for optname in opts_keys:
        optdesc = OPT_DESC[optname]
        print("{0:<16s} {1:s}".format(optname, optdesc[0]))
        if len(optdesc) > 1:  # params
            for (param, pdesc) in optdesc[1].items():
                print("{0:2s} {1:<16s} {2:s}".format('', param, pdesc[1]))
...
def parse_argv():
    ...
    devices = ([
        i for i in files
        if i.startswith('/dev/') or i.startswith('/tmp/fakemidi')
    ...
    if '--note-pressures' in options:
        ...
        if paramtab:
            ...
            # up to this value, it is consider as soft
            OPTIONS[NOTE_PRESSURE_MIDDLE_IDX] = 40
